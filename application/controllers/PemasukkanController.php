...
class PemasukkanController extends CI_Controller
{
	...
	public function simpan()
	{
		...
		if ($this->form_validation->run() == FALSE) {
            ...
            $last_forecast = $forecast[0]->jumlah_ramalan;
            $ramal9 = round(0.9*$stok+0.1*$last_forecast);
            $ramal8 = round(0.9*$stok+0.1*$last_forecast);
            $ramal7 = round(0.9*$stok+0.1*$last_forecast);
            $ramal6 = round(0.9*$stok+0.1*$last_forecast);
            $ramal5 = round(0.9*$stok+0.1*$last_forecast);
            $ramal4 = round(0.9*$stok+0.1*$last_forecast);
            $ramal3 = round(0.9*$stok+0.1*$last_forecast);
            $ramal2 = round(0.9*$stok+0.1*$last_forecast);
            $ramal1 = round(0.9*$stok+0.1*$last_forecast);
